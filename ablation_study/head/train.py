...
import ablation_study.head.our_model_fully as our_model_conv
import ablation_study.head.our_model_fully as our_model_only_glob
...
def get_model(hparams):
    ...
    if hparams['model'] == 'ours':
        if hparams['head'] == 'global':
            model = ClassifierNetwork(bands)
        elif hparams['head'] == 'fully':
            model = our_model_conv.ClassifierNetwork(bands)
        elif hparams['head'] == 'only_global':
            model = our_model_only_glob.ClassifierNetwork(bands)
        else:
...
def get_args():
    ...
    parser.add_argument("--head", default="global", type=str)

...
class DeepHsAblationStudyModule(DeepHsModule):
    ...
    def configure_optimizers(self):
        if self.hparams['optimizer'] == 'best':
            self.optimizer = AdaBound(self.model.parameters(), lr=1e-2)
        ...
        if self.hparams['optimizer'] == 'adabound_defaultlr':
        ...
        if self.hparams['optimizer'] == 'adabound_withoutschedule':
            self.optimizer = AdaBound(self.model.parameters())
        ...
        if self.hparams['optimizer'] == 'sgd':
            self.optimizer = torch.optim.SGD(self.model.parameters(), 1e-5)
            self.scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(self.optimizer, verbose=True)
...
def get_args():
    ...
    parser.add_argument("--optimizer", default="best", type=str)
